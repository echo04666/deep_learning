...
3. 系统先将生成文本进行**句子拆分**（每句作为独立检测单元）。
4. 系统将每个句子送入 **内容安全检测** 模块：
   - 可选：敏感词词典快速扫描；
   - 必选：基于微调分类模型的 **Text Classification**，输出标签与置信度。
5. 系统逐句展示检测结果，并支持用户在检测步骤中对单句执行编辑、删除、新增与重检。
6. 系统聚合结果并展示：
   - 生成正文；
   - 逐句检测结果（例如：无毒 / 有毒、分数、命中关键词说明）；
...
flowchart LR
  ...
  subgraph safety [Safety]
    splitSentence[SentenceSplit]
    dictScan[OptionalDictScan]
    getCls[get_text_classification_pipeline]
    clsPipe[HF_text_classification]
    reviewEdit[SentenceEdit_Delete_Add_Recheck]
  end
  input --> getGen
  getGen --> genPipe
  genPipe --> splitSentence
  splitSentence --> dictScan
  dictScan --> getCls
  getCls --> clsPipe
  clsPipe --> reviewEdit
  reviewEdit --> display
...
| 句子拆分 | `generated_text` | `sentences: list[str]` |
| 词典扫描（可选） | `sentence`、词表 | 该句是否命中、命中词列表（若有） |
| 分类检测 | `sentence` | 该句 `label`、`score`（及多标签时的策略说明） |
| 人工修订与复检 | 逐句检测结果、用户编辑内容 | 更新后的逐句检测结果 |
